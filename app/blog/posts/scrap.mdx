...
export const updated = new Date(2025, 0, 21);
...
1. **Block-based coding** &ndash; While Scrap encourages transition to text-based programming, it supports both block an
   text editors.
2. **Strongly typed** &ndash; In text editor, Scrap supports TypeScript type annotations, which are used to provide type safety.
...
<q>ScrapScript</q> is a code name for the problem of the incompatibility. It aims to design an ECMAScript subset, which can be converted to blocks and back **as easy as possible**.

It's been obvious since the very beginning that ScrapScript needs to be typed, as not all blocks are compatible by shape. Previously, this was handled via JSDoc but [when the Monaco Editor was introduced in Scrap 4](https://github.com/tomaswrobel/scrap/releases/tag/v4.0.0), TypeScript was way more helpful.
...
Thanks to [#158](https://github.com/tomaswrobel/scrap/issues/158), I realised that `Date.prototype.getTime()` **can** be implemented.
...
As Scrap doesn't have a VM, somehow it needs to delay block execution (like Scratch does), and control code execution. Compiler handles most of that, but still, some things must be done at runtime – setters, getters and throwing uncatchable errors do the job.

Scrap comes up with a SWC plugin, which provides users better coding experience, as their code can be <q>less ugly</q>. That means, if one includes [Scrap Engine](https://npmjs.com/package/scrap-engine) and use just the code written in IDE, it won't work.

Let's see how engine and SWC cooperate:
...
- Setters / getters
    ...
    - In IDE, users use more familiar syntax: `self.x = 10`
- Control flow
    - When Scrap engine needs to, or is told to stop the project, it throws `Scrap.StopError`
...
- On-screen variables
    ...
    - SWC allows users to declare them inside `Variables` interface and rewrites the setters and getters 
...
Scrap has a great potential to become a great tool, doesn't it? 

If you totally agree, and, by chance, you are a developer, consider [joining the team](/blog/scrap-contribution-needed.mdx).